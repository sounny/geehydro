// Earth Engine Strahler stream order from flow accumulation network
...
function strahlerOrder(flowDir, streams, iterations) {
  iterations = iterations || 100;
  var order = streams.mask().multiply(1).rename('order');
  var kernel = ee.Kernel.square(1);
  var offsets = [
    [-1, -1], [-1, 0], [-1, 1],
    [ 0, -1],           [ 0, 1],
  ...
  var inflowDir = [2, 4, 8, 1, 16, 128, 64, 32];
  for (var step = 0; step < iterations; step++) {
    var ordNeigh = order.neighborhoodToBands(kernel);
    var dirNeigh = flowDir.neighborhoodToBands(kernel);
    var upList = [];
    for (var k = 0; k < 8; k++) {
      var row = offsets[k][0];
      var col = offsets[k][1];
      var dirBand = 'flowDirection_' + row + '_' + col;
      var ordBand = 'order_' + row + '_' + col;
      var mask = dirNeigh.select(dirBand).eq(inflowDir[k]);
      var contrib = ordNeigh.select(ordBand).updateMask(mask);
      upList.push(contrib);
    }
    var stack = ee.ImageCollection(upList).toBands();
    var maxUp = stack.reduce(ee.Reducer.max());
    var countMax = stack.eq(maxUp).reduce(ee.Reducer.sum());
    var newOrder = maxUp.add(countMax.gte(2));
    order = order.where(streams, newOrder);
  }
  return order;
}
...
var runBtn = ui.Button('Compute Stream Order');
var panel = ui.Panel([
...
runBtn.onClick(function() {
  ...
  var order = strahlerOrder(fdir, streams, 100);
  Map.clear();
  Map.addLayer(order, {min: 1, max: 5, palette: ['00ff00', '0000ff', 'ff0000', 'ffff00', '00ffff']}, 'Strahler Order');
